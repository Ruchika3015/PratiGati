...
export default function TruckerPredictionsPage() {
  const { t } = useI18n();

  return (
    <DashboardLayout role="trucker" title={t('predictions')} subtitle={t('predictionsSubtitle')}>
      <div className="p-4 lg:p-6">
        <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
          <div className="flex items-start gap-3">
            <Brain className="mt-0.5 h-5 w-5 shrink-0 text-primary" />
            <div>
              <p className="text-sm font-bold">{t('aiPoweredPredictions')}</p>
              <p className="text-xs text-muted-foreground">{t('predictionsDesc')}</p>
            </div>
          </div>
        </div>
            ...
            return (
              <div key={pred.id} className="rounded-lg border border-border bg-card p-4">
                ...
                <div className="mt-3 flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">{t('historicalEmptyRate')} <span className="font-bold text-amber-500">{pred.avgEmptyRate}%</span></span>
                  <span className="text-muted-foreground">{pred.historicalTrips} {t('historicalTrips')}</span>
                </div>
              </div>
            );
          })}
